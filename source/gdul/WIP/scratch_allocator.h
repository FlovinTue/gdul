...
namespace sa_detail {
constexpr std::size_t Scratch_Pad_Default_Align(alignof(std::max_align_t));
...
{	
public:
	static constexpr std::size_t AlignedStorageSize = sa_detail::align_value(StorageSize, sa_detail::Scratch_Pad_Default_Align);
...
private:
	...
	std::uint8_t alignas(sa_detail::Scratch_Pad_Default_Align) m_bytes[AlignedStorageSize];

	std::atomic<sizes> m_sizes;
};
template<std::size_t StorageSize>
inline scratch_pad<StorageSize>::scratch_pad()
	: m_bytes{}
	, m_sizes({ 0, 0 })
{
}
template<std::size_t StorageSize>
inline scratch_pad<StorageSize>::~scratch_pad() noexcept
{
	assert(m_sizes.load(std::memory_order_acquire).at == 0 && "scratch_pad destroyed with items in use");
}
template<std::size_t StorageSize>
inline void* scratch_pad<StorageSize>::allocate(std::size_t count, std::size_t align)
{
...
template<std::size_t StorageSize>
inline void scratch_pad<StorageSize>::deallocate(void* p, std::size_t count)
{
...
template<std::size_t StorageSize>
inline std::size_t scratch_pad<StorageSize>::remaining_storage() const noexcept
{
	return AlignedStorageSize - m_sizes.load(std::memory_order_acquire).at;
}
template<std::size_t StorageSize>
inline bool scratch_pad<StorageSize>::owns(const void* p) const
{
	return !(p < &m_bytes[0]) && p < &m_bytes[StorageSize];
}
template<std::size_t StorageSize>
inline void scratch_pad<StorageSize>::reset()
{
	m_sizes.store({ 0, 0 }, std::memory_order_release);
}
template<std::size_t StorageSize>
template<class T, class ParentAllocator>
inline scratch_allocator<T, ParentAllocator> scratch_pad<StorageSize>::create_allocator(const ParentAllocator& parent)
{
